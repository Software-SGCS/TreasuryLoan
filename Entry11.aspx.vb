...
Public Class Entry11
    ...
    Protected WithEvents TxtttNo As System.Web.UI.WebControls.TextBox
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not IsPostBack Then
            ...
            CMD = New SqlCommand("SELECT Mas_Refno FROM MasterDocs Where Mas_InTrm='TT'", CON)
            ...
            If count1 <> 0 Then
                ...
                For Each drt2 In DS.Tables("MasterDocs").Rows
                    ...
                    Calendar2.Visible = False
                    Calendar3.Visible = False
                    ADA1 = New SqlDataAdapter("SELECT * FROM TTMas Where Mas_refno='" & Trim(TxtRefno.Text) & "'", CON)

                    DS = New DataSet
                    ADA1.Fill(DS, "TTMas")

                    count1 = DS.Tables("TTMas").Rows.Count

                    If count1 <> 0 Then

                        For Each drt3 In DS.Tables("TTMas").Rows
                            TxtAdv.Text = Trim(drt3("Mas_Des"))
                            TxtAppDate.Text = Format(CDate(Trim(drt3("Mas_Appdate"))), "dd/MM/yyyy")
                            TxtRecDate.Text = Format(CDate(Trim(drt3("Mas_Remdate"))), "dd/MM/yyyy")
                            TxtExpDt.Text = Format(CDate(Trim(drt3("Mas_ExpDate"))), "dd/MM/yyyy")
                            TxtttNo.Text = Trim(drt3("Mas_TNo"))
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        ...
        Dim Cmd As New SqlCommand("SELECT * FROM TTMas", CON)
        Cmd.ExecuteNonQuery()

        ADA = New SqlDataAdapter("SELECT * FROM TTMas Where Mas_RefNo='" & Trim(TxtRefno.Text) & "'", CON)
        DS = New DataSet
        ADA.Fill(DS, "TTMas")
        ...
        If count1 <> 0 Then
            ...
            ss = "UPDATE TTMas SET Mas_des='" & Trim(TxtAdv.Text) & "',Mas_AppDate='" & Format(CDate(Trim(TxtAppDate.Text)), "MM/dd/yyyy") & "',Mas_Remdate='" & Format(CDate(Trim(TxtRecDate.Text)), "MM/dd/yyyy") & "' WHERE Mas_Refno='" & Trim(TxtRefno.Text) & "'"
            Cmd.CommandText = ("UPDATE TTMas SET Mas_Des='" & Trim(TxtAdv.Text) & "',Mas_Appdate='" & Mid(Trim(TxtAppDate.Text), 4, 2) + "/" + Mid(Trim(TxtAppDate.Text), 1, 2) + "/" + Mid(Trim(TxtAppDate.Text), 7, 4) & "',Mas_Remdate='" & Mid(Trim(TxtRecDate.Text), 4, 2) + "/" + Mid(Trim(TxtRecDate.Text), 1, 2) + "/" + Mid(Trim(TxtRecDate.Text), 7, 4) & "',Mas_Expdate='" & Mid(Trim(TxtExpDt.Text), 4, 2) + "/" + Mid(Trim(TxtExpDt.Text), 1, 2) + "/" + Mid(Trim(TxtExpDt.Text), 7, 4) & "',Mas_Tno='" & Trim(TxtttNo.Text) & "' WHERE Mas_Refno='" & Trim(TxtRefno.Text) & "'")
            Cmd.ExecuteNonQuery()
        Else
            ...
            Cmd.CommandText = ("INSERT INTO TTMas(Mas_Refno,Mas_des,Mas_AppDate,Mas_RemDate,Mas_ExpDate,Mas_Tno) VALUES ('" & Trim(TxtRefno.Text) & "','" & Trim(TxtAdv.Text) & "','" & Mid(Trim(TxtAppDate.Text), 4, 2) + "/" + Mid(Trim(TxtAppDate.Text), 1, 2) + "/" + Mid(Trim(TxtAppDate.Text), 7, 4) & "','" & Mid(Trim(TxtRecDate.Text), 4, 2) + "/" + Mid(Trim(TxtRecDate.Text), 1, 2) + "/" + Mid(Trim(TxtRecDate.Text), 7, 4) & "','" & Mid(Trim(TxtExpDt.Text), 4, 2) + "/" + Mid(Trim(TxtExpDt.Text), 1, 2) + "/" + Mid(Trim(TxtExpDt.Text), 7, 4) & "','" & Trim(Trim(TxtttNo.Text)) & "')")
            Cmd.ExecuteNonQuery()
            ss = Format(Today(), "MM/dd/yyyy")
            Cmd.CommandText = ("UPDATE MasterDocs SET Mas_EntryTt='Ok',Mas_EntryTTDt='" & ss & "' WHERE Mas_Refno='" & Trim(TxtRefno.Text) & "'")
            Cmd.ExecuteNonQuery()
            ...
            For Each drt2 In DS.Tables("MasterDocs").Rows
                ...
                If count1 <> 0 Then
                    ...
                    For Each drt3 In DS.Tables("TTMas").Rows
                        ...
                        TxtRecDate.Text = Format(CDate(Trim(drt3("Mas_Remdt"))), "dd/MM/yyyy")
                        TxtExpDt.Text = Format(CDate(Trim(drt3("Mas_ExpDt"))), "dd/MM/yyyy")
                        TxtttNo.Text = Trim(drt3("Mas_TNo"))
                        ' LstDraft.SelectedItem.Selected = Trim(drt3("Mas_TTType"))
                    Next
                Else
                    TxtAdv.Text = ""
                    TxtAppDate.Text = ""
