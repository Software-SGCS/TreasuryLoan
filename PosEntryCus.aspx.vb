...
#End Region
    ...
    Public Sub BindTheGrid()
        ...
        Dim cmdCustomers As SqlCommand = New SqlCommand("Select id,LeCode,LeName,LeAdd1,LeEmail From Lender order by LeSName", CON)
    ...
    Sub MyDataGrid_DeleteCommand(ByVal s As Object, ByVal e As DataGridCommandEventArgs)
        ...
        Dim DeleteCmd As String = "DELETE from Lender Where id = @Id"
    ...
    Private Sub BttIns1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BttIns1.Click
        ...
        ADA = New SqlDataAdapter("SELECT * FROM Lender WHERE LeCode = '" & Trim(TxtComNo.Text) & "'", CON)
        DS = New DataSet
        ...
        count1 = DS.Tables("Lender").Rows.Count
        If count1 <> 0 Then

            CMD.Connection = CON
            CMD.CommandText = ("UPDATE Lender SET LeCode='" & Trim(TxtComNo.Text) & "',LeName='" & Trim(TxtComNam.Text) & "',LeAdd1= '" & TxtComAdd.Text & "',LeAdd2= '" & TxtComAdd1.Text & "',LeContact= '" & TxtComPer.Text & "',LeEmail= '" & Txtemail.Text & "',LeSName= '" & TxtVat.Text & "',LeDesignation= '" & TxtCrd.Text & "' WHERE LeCode='" & TxtComNo.Text & "'")
            CMD.ExecuteNonQuery()

        Else
            CMD.Connection = CON
            CMD.CommandText = ("INSERT INTO Lender(LeCode,LeName,LeAdd1,LeAdd2,LeContact,LeEmail,LeSName,LeOpenLKR,LeOpenUSD,LeCurrentLKR,LeCurrentUSD,LeAdd3,LeDesignation) VALUES ('" & Trim(TxtComNo.Text) & "','" & Trim(TxtComNam.Text) & "','" & TxtComAdd.Text & "','" & TxtComAdd1.Text & "','" & TxtComPer.Text & "','" & Txtemail.Text & "','" & TxtVat.Text & "',0,0,0,0,' ','" & TxtCrd.Text & "')")
            CMD.ExecuteNonQuery()
        End If
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        ADA = New SqlDataAdapter("SELECT * FROM Lender WHERE LeCode = '" & TxtComNo.Text & "'", CON)
        DS = New DataSet
        ...
        If count1 <> 0 Then
            ...
            For Each drt2 In DS.Tables("Lender").Rows
                StrComno = drt2("LeCode")
                StrComAdd = IIf(IsDBNull(drt2("LeAdd1")) = True, "", drt2("LeAdd1"))
                StrComAdd1 = IIf(IsDBNull(drt2("LeAdd2")) = True, "", drt2("LeAdd2"))
                StrComNam = IIf(IsDBNull(drt2("LeName")) = True, "", drt2("LeName"))
                StrComPer = IIf(IsDBNull(drt2("LeContact")) = True, "", drt2("LeContact"))
                StrComEmail = IIf(IsDBNull(drt2("LeEmail")) = True, "", drt2("LeEmail"))
                StrComVat = IIf(IsDBNull(drt2("LeSName")) = True, "", drt2("LeSName"))
                StrComCrd = IIf(IsDBNull(drt2("LeDesignation")) = True, "Blank ", drt2("LeDesignation"))
                StrComTelno = IIf(IsDBNull(drt2("LeTel")) = True, "Blank ", drt2("LeTel"))
