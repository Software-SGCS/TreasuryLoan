...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not IsPostBack Then
            ...
            CMD = New SqlCommand("SELECT Mas_Refno FROM MasterDocs Where Mas_InTrm='BD'", CON)
            ...
            If count1 <> 0 Then
                ...
                For Each drt2 In DS.Tables("MasterDocs").Rows
                    ...
                    Calendar2.Visible = False
                    Calendar3.Visible = False
                    ADA1 = New SqlDataAdapter("SELECT * FROM BDMas Where Mas_refno='" & Trim(TxtRefno.Text) & "'", CON)

                    DS = New DataSet
                    ADA1.Fill(DS, "BDMas")

                    count1 = DS.Tables("BDMas").Rows.Count

                    If count1 <> 0 Then

                        For Each drt3 In DS.Tables("BDMas").Rows
                            TxtAdv.Text = Trim(drt3("Mas_Des"))
                            TxtAppDate.Text = Format(CDate(Trim(drt3("Mas_Appdate"))), "dd/MM/yyyy")
                            TxtRecDate.Text = Format(CDate(Trim(drt3("Mas_ReCdate"))), "dd/MM/yyyy")
                            TxtExpDt.Text = Format(CDate(Trim(drt3("Mas_ExpDate"))), "dd/MM/yyyy")
                            TxtttNo.Text = Trim(drt3("Mas_BNo"))
    ...
    Private Sub BtnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSave.Click
        ...
        Dim Cmd As New SqlCommand("SELECT * FROM BDMas", CON)
        Cmd.ExecuteNonQuery()

        ADA = New SqlDataAdapter("SELECT * FROM BDMas Where Mas_RefNo='" & Trim(TxtRefno.Text) & "'", CON)
        DS = New DataSet
        ADA.Fill(DS, "BDMas")
        ...
        If count1 <> 0 Then
            ...
            ss = "UPDATE BDMas SET Mas_des='" & Trim(TxtAdv.Text) & "',Mas_AppDate='" & Format(CDate(Trim(TxtAppDate.Text)), "MM/dd/yyyy") & "',Mas_Remdate='" & Format(CDate(Trim(TxtRecDate.Text)), "MM/dd/yyyy") & "' WHERE Mas_Refno='" & Trim(TxtRefno.Text) & "'"
            Cmd.CommandText = ("UPDATE BDMas SET Mas_Des='" & Trim(TxtAdv.Text) & "',Mas_Appdate='" & Mid(Trim(TxtAppDate.Text), 4, 2) + "/" + Mid(Trim(TxtAppDate.Text), 1, 2) + "/" + Mid(Trim(TxtAppDate.Text), 7, 4) & "',Mas_Recdate='" & Mid(Trim(TxtRecDate.Text), 4, 2) + "/" + Mid(Trim(TxtRecDate.Text), 1, 2) + "/" + Mid(Trim(TxtRecDate.Text), 7, 4) & "',Mas_Expdate='" & Mid(Trim(TxtExpDt.Text), 4, 2) + "/" + Mid(Trim(TxtExpDt.Text), 1, 2) + "/" + Mid(Trim(TxtExpDt.Text), 7, 4) & "',Mas_bno='" & Trim(TxtttNo.Text) & "' WHERE Mas_Refno='" & Trim(TxtRefno.Text) & "'")
            Cmd.ExecuteNonQuery()
        Else
