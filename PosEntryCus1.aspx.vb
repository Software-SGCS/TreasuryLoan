...
Public Class PosEntryCus1
    ...
    Dim StrComno, StrComEmail, StrComNam, StrComVat, StrComPer, StrComAdd1, StrComAdd, StrComTelno, StrComCrd As String
    ...
    Protected WithEvents TxtComAdd1 As System.Web.UI.WebControls.TextBox
...
#End Region
    ...
    Public Sub BindTheGrid()
        ...
        Dim cmdCustomers As SqlCommand = New SqlCommand("Select id,BrCode,BrName,BrAdd1,BrEmail From Borrower order by BrSName", CON)
    ...
    Sub MyDataGrid_DeleteCommand(ByVal s As Object, ByVal e As DataGridCommandEventArgs)
        ...
        Dim DeleteCmd As String = "DELETE from Borrower Where id = @Id"
    ...
    Private Sub BttIns1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BttIns1.Click
        ...
        ADA = New SqlDataAdapter("SELECT * FROM Borrower WHERE BrCode = '" & Trim(TxtComNo.Text) & "'", CON)
        DS = New DataSet
        ...
        ADA.Fill(DS, "Borrower")

        count1 = DS.Tables("Borrower").Rows.Count
        If count1 <> 0 Then

            CMD.Connection = CON
            CMD.CommandText = ("UPDATE Borrower SET BrCode='" & Trim(TxtComNo.Text) & "',BrName='" & Trim(TxtComNam.Text) & "',BrAdd1= '" & TxtComAdd.Text & "',BrAdd2= '" & TxtComAdd1.Text & "',BrContact= '" & TxtComPer.Text & "',BrEmail= '" & Txtemail.Text & "',BrSName= '" & TxtVat.Text & "',BrDesignation= '" & TxtCrd.Text & "' WHERE BrCode='" & TxtComNo.Text & "'")
            CMD.ExecuteNonQuery()

        Else
            CMD.Connection = CON
            CMD.CommandText = ("INSERT INTO Borrower(BrCode,BrName,BrAdd1,BrAdd2,BrContact,BrEmail,BrSName,BrOpenLKR,BrOpenUSD,BrCurrentLKR,BrCurrentUSD,BrAdd3,BrDesignation) VALUES ('" & Trim(TxtComNo.Text) & "','" & Trim(TxtComNam.Text) & "','" & TxtComAdd.Text & "','" & TxtComAdd1.Text & "','" & TxtComPer.Text & "','" & Txtemail.Text & "','" & TxtVat.Text & "',0,0,0,0,' ','" & TxtCrd.Text & "')")
            CMD.ExecuteNonQuery()
        End If
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        ADA = New SqlDataAdapter("SELECT * FROM Borrower WHERE BrCode = '" & TxtComNo.Text & "'", CON)
        DS = New DataSet
        ...
        If count1 <> 0 Then
            ...
            For Each drt2 In DS.Tables("Borrower").Rows
                StrComno = drt2("BrCode")
                StrComAdd = IIf(IsDBNull(drt2("BrAdd1")) = True, "", drt2("BrAdd1"))
                StrComAdd1 = IIf(IsDBNull(drt2("BrAdd2")) = True, "", drt2("BrAdd2"))
                StrComNam = IIf(IsDBNull(drt2("BrName")) = True, "", drt2("BrName"))
                StrComPer = IIf(IsDBNull(drt2("BrContact")) = True, "", drt2("BrContact"))
                StrComEmail = IIf(IsDBNull(drt2("BrEmail")) = True, "", drt2("BrEmail"))
                StrComVat = IIf(IsDBNull(drt2("BrSName")) = True, "", drt2("BrSName"))
                StrComCrd = IIf(IsDBNull(drt2("BrDesignation")) = True, "Blank ", drt2("BrDesignation"))
                StrComTelno = IIf(IsDBNull(drt2("BrTel")) = True, "Blank ", drt2("BrTel"))
